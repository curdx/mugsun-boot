...
public final class RuntimeSql {
	...
	public static String upsertTableColumn(SqlDialect d) {
		...
		if (d.oracleFamily()) {
			return "MERGE INTO sys_table_column t "
		...
		return "insert into sys_table_column (id, user_id, table_key, config_json, create_time, update_time, is_deleted) "
			+ "values (?, ?, ?, ?, " + ts + ", " + ts + ", 0) "
	...
	public static String upsertWorkbenchShortcut(SqlDialect d) {
		String ts = d.currentTimestamp();
		if (d.oracleFamily()) {
			return "MERGE INTO sys_workbench_shortcut t "
		...
		return "insert into sys_workbench_shortcut (id, user_id, config_json, create_time, update_time, is_deleted) "
			+ "values (?, ?, ?, " + ts + ", " + ts + ", 0) "
	...
	public static String upsertSerialRecord(SqlDialect d) {
		String ts = d.currentTimestamp();
		if (d.oracleFamily()) {
			return "MERGE INTO sys_serial_number_record t "
		...
		return "insert into sys_serial_number_record (id, serial_code, record_date, last_number, last_time, gen_count, create_time, is_deleted) "
			+ "values (?, ?, ?, ?, ?, ?, " + ts + ", 0) "
	...
	public static String updateSerialLastMonotonic(SqlDialect d) {
		...
		return "update sys_serial_number set last_number = greatest(coalesce(last_number, 0), ?), "
	...
	public static String upsertNoticeReadPg() {
		...
		return "insert into sys_notice_read (id, notice_id, user_id, read_count, first_time, last_time, create_time, update_time, is_deleted) "
			+ "values (?, ?, ?, 1, " + ts + ", " + ts + ", " + ts + ", " + ts + ", 0) "
			+ "on conflict (notice_id, user_id) where is_deleted = 0 "
			+ "do update set read_count = sys_notice_read.read_count + 1, last_time = " + ts + ", update_time = " + ts + " "
	...
	public static String bumpNoticeRead(SqlDialect d) {
		String ts = d.currentTimestamp();
		return "update sys_notice_read set read_count = read_count + 1, last_time = " + ts + ", update_time = " + ts
