...
SELECT 1093000000000000014, p.id, '圈选规则查看', 'F', 'sys:track-visual:list', 10, SYSDATE, 0
FROM sys_menu p
...
INSERT INTO sys_menu (id, parent_id, menu_name, menu_type, permission, sort, create_time, is_deleted)
SELECT 1093000000000000015, p.id, '圈选规则管理', 'F', 'sys:track-visual:edit', 11, SYSDATE, 0
FROM sys_menu p
WHERE p.path = '/track/app' AND p.is_deleted = 0
