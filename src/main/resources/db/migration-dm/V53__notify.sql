...
CREATE TABLE sys_notify_template (
	...
	channels        VARCHAR(128) DEFAULT 'in_app' NOT NULL,
...
-- 渠道配置（平台级基础设施，与 sys_sms 平台级通道先例一致；config CLOB 禁类名，按渠道编码映射 Java 配置类）
...
COMMENT ON COLUMN sys_notify_channel.config IS '渠道非敏感配置 CLOB（mail: host/port/username/from；in_app/sms 为空）';
...
CREATE TABLE sys_notify_record (
	...
	status           VARCHAR(16)  DEFAULT 'INIT' NOT NULL,
...
ALTER TABLE sys_user ADD email VARCHAR(128);
...
(1090000000000000010, 'welcome', '新用户欢迎通知', '欢迎 ${name}',
 ...
 'name', 'in_app', 1, '新建用户成功后多渠道触达（V26 sys_message_template welcome 的统一模板化）', SYSDATE, 0);
...
(1090000000000000021, 'in_app', '站内信', 1, '{}', '委托消息中心 MessageService（保留实时推送）', SYSDATE, 0),
(1090000000000000022, 'mail', '邮件', 0, '{"host":"smtp.example.com","port":465,"username":"","from":"noreply@mugsun.com"}', 'SMTP 凭据密码存 secret 列（SM4 加密）', SYSDATE, 0),
(1090000000000000023, 'sms', '短信', 1, '{}', '委托 SmsService（复用 sys_sms 库表热配置，不手写厂商签名）', SYSDATE, 0);
...
(900012, '通知重试扫描间隔(毫秒)', 'notify.retry.scan-interval-ms', '60000', '调度按此间隔扫描 FAILURE 且到期的发送流水', SYSDATE, 0),
(900013, '通知重试最大次数', 'notify.retry.max-times', '3', '单条流水最大重试次数，达到后转 DEAD 死信', SYSDATE, 0),
(900014, '通知重试退避基数(毫秒)', 'notify.retry.backoff-ms', '300000', '线性退避：下次重试时间 = 当前 + 基数 × 已重试次数', SYSDATE, 0);
